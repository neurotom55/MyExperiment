...
function createTrial() {
    var stimuli = [];
    var first_target = Math.floor(Math.random() * 10).toString();  // First target as a random digit
    var second_target = Math.floor(Math.random() * 10).toString(); // Second target as a different random digit
...
var trial = {
    ...
    on_start: function(trial) {
        trial.timeline = createTrial().map(function(item) {
            return {
                type: 'html-keyboard-response',
